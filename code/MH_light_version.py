...
#提取所有的persName和placeName
    ...
    for child in node:
        if child.tag=='{http://www.tei-c.org/ns/1.0}persName':
            ...
            '''
            #修改all_persName成light版本
            the_pattern='The'
            majesty_pattern='(?i)((her)|(his) majesty)'
            for p in all_persName[:]:
                #如果有小写
                has_lower=False
                words=p.split(' ')       
                for w in words:
                    if w.islower()==True:
                        while p in all_persName:
                           all_persName.remove(p)
                        #print(w)

                #如果包含The
                if len(re.findall(the_pattern, persName))!=0:
                     #如果p还在里面，没被删除，就remove掉
                     while p in all_persName:
                        all_persName.remove(p)
                                            
                #如果是无论大小写的her majesty/his majesty
                if len(re.findall(majesty_pattern, persName))!=0:
                    while p in all_persName:
                        all_persName.remove(p) 
                #如果是King或者Queen
                if persName=='King' or persName=='Queen':            
                    while p in all_persName:
                        all_persName.remove(p)
           '''
...
def light_version():
    ...
    for p in all_persName[:]:
        ...
        #如果包含The
        if len(re.findall(the_pattern, p))!=0:
             #如果p还在里面，没被删除，就remove掉
             while p in all_persName:
                all_persName.remove(p)
                                    
        #如果是无论大小写的her majesty/his majesty
        if len(re.findall(majesty_pattern, p))!=0:
            while p in all_persName:
                all_persName.remove(p) 
        #如果是King或者Queen
...
#将文本与实体类型对应起来
    ...
    space_pattern=r'\s{1,}'
    text_str=re.sub(space_pattern,' ',text_str)
    ...
    for p in pers_matches[:]:
        ...
        #如果有小写
        has_lower=False
        words=persName.split(' ')       
        for w in words:
            if w.islower()==True:
                while p in pers_matches:
                   pers_matches.remove(p)
                #print(w)
...
if __name__ == "__main__":
    ...
    '''
    file='AR-HAM-00001-00001-00001-00027.xml'
    all_node=[]
    all_persName=[]
    all_placeName=[]
